...
String Wert[14][15];  // Vor und hinter "=", erster index zeile, zweiter index 1=davor, 2=dahinter
int line_count;
const char *ssid;
const char *pwd;
float volt = 0;
...
float getParams(String Param) 
{
  ...
  for (int n = 1; n <= line_count; n++) //Modus holen
  {
    if (Wert[n][1] == "Modus")
    {      
      Modus = Wert[n][2];
  ...
  {
    volt = ((value[10] * 256 + value[11])*0.01) ;
  }
